...
export interface RateLimitOptions {
  ...
  key?: string; // Optional custom key for the rate limit
}
...
export class RateLimitService {
  static async checkRateLimit(
    ...
    try {
      ...
      const scriptResult = await redis.eval(
        RATE_LIMIT_SCRIPT,
        [counterKey],
        [options.limit, options.windowSeconds, Date.now()]
      );
   ...
   */
  private static resolveKeyTemplate(template: string, subject: JWTClaims): string {
    return template.replace(/\{subject\.([^}]+)\}/g, (match, claimKey) => {
      // Basic resolution for now, can be extended for nested properties
      if (claimKey === 'sub') return subject.sub;
      if (claimKey === 'role') return subject.role;
      if (claimKey === 'tenant') return subject.tenant || 'default';
