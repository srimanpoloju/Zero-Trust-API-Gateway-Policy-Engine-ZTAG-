...
export async function gatewayRoutes(fastify) {
    ...
    fastify.decorateReply('sendError', function (code, message, statusCode, details) {
    ...
    fastify.get('/health', async (request, reply) => {
        try {
            ...
            reply.status(503).send({ status: 'error', message: 'Gateway unhealthy', error: error.message });
        }
    });
    ...
    fastify.all('/api/*', async (request, reply) => {
        ...
        try {
            ...
            try {
                ...
                request.audit.reason = `Invalid token: ${error.message}`;
                return reply.sendError('INVALID_TOKEN', `Invalid token: ${error.message}`, 403);
            }
            // 2. Dynamic Routing
            ...
            if (decisionResponse.obligations?.rateLimit) {
                ...
                rateLimitInfo = await RateLimitService.checkRateLimit(claims, {
            ...
            // 5. Proxy Request
            const proxyResponse = await ProxyService.proxyRequest(matchedRoute.targetUrl, {
                method: request.method,
                path: downstreamPath,
                headers: request.headers,
                body: request.rawBody, // Use raw body if available
                query: request.query,
            }, request.id, request.ip, matchedRoute.stripHeaders);
            ...
            request.audit.error = error.message;
            statusCode = 500;
            request.audit.statusCode = statusCode;
