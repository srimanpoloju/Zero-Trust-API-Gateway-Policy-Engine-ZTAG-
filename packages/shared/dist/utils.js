...
export class JWTUtils {
    ...
    static extractRole(claims) {
        return claims.role;
    }
    static extractTenant(claims) {
        return claims.tenant;
    }
    static extractScopes(claims) {
        return claims.scopes || [];
    }
}
// Policy Evaluation Utilities
export class PolicyEvaluator {
    static evaluateCondition(subject, condition) {
    ...
    static matchMethod(methods, requestMethod) {
...
export class DateTimeUtils {
    ...
    static getWindowStart(windowSeconds) {
        const now = new Date();
        const windowMs = windowSeconds * 1000;
        // Calculate start of the current window
        return new Date(Math.floor(now.getTime() / windowMs) * windowMs);
    }
    static getResetTime(windowSeconds) {
